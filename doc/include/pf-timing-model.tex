...
\]
which is also the expansion of the start times of $\{ \task_k: k \in \iTask_{>i}, \; \size{\iTask_{k<}} = 1 \}$ (see \sref{application-model}). The max operation is required for $\{ \S_i(\o): \size{\iTask_{>i}} > 1 \}$ and is performed via an additional spectral expansion based on the expansions of $\{ \E_j(\o): j \in \iTask_{>i} \}$. The spectral expansion of the end time $\E_\text{sink}(\o)$ of the sink node yields an analytical expression of the total execution time of the application, which can be further analyzed in a straightforward manner.
