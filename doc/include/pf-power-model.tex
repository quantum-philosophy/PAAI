...
\end{equation}
where $\PT_j(\o)$ is the power mode (the power consumption) of the $j$th task. $\I_\event(\o)$ denotes the indicator function of an event $A \in \F$ (see \sref{uncertainty-model}), which is equal to one if $\o \in \event$ and to zero, otherwise. The time-dependent events $\event_j(\t)$ in \eref{processor-power} are defined as
\[
