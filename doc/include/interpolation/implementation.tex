...
\point{Line~7:} \token{Evaluate} uses the surrogate constructed so far in order
to calculate an approximation to the values of \token{target} obtained on
line~6; \token{Evaluate} is \aref{evaluate}, and it will be described
separately.
...
the surpluses; see \sref{adaptivity}.
...
Let us now turn to the usage phase of interpolation. The corresponding
pseudocode is given in \aref{evaluate} called \token{Evaluate}; this algorithm
is also involved in the construction phase; see \aref{construct}, line~7. The
main steps of \token{Evaluate} are given below.
