...
exercised only at the nodes of the sparse grid underpinning \eref{smolyak}:
...
The sparsity and gradual refinement of Smolyak's approach---shown in
...
\bigcup_{|\vi|_1 = l} \Delta\X^\vi$ in \eref{smolyak-grid-incremental} gets
...
Lastly, we would like to impose one additional property on $\tensor{i}$ in order
to enable hierarchical implementations of \eref{smolyak-incremental}. Namely,
...
of $\f$ are constructed using \eref{tensor-surplus-1d} and \eref{tensor-delta}
