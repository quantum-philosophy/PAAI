...
in \sref{uncertain-parameters}. Second, an interpolant of the resulting
...
criterion given in \eref{error}. Here the absolute value $|\cdot|$ should be
...
functions given in \sref{basis-functions}, $\w_{00} = 1$ and, for $i > 0$,
\[
  \w_{ij} = \begin{cases}
    \frac{2}{\n_i + 1}, & \text{if } j \in \{ 0, \n_i - 1 \}, \\
    \frac{1}{\n_i + 1}, & \text{otherwise},
