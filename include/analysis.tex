\updated{In \sref{modeling}, we formalized the uncertainty affecting electronic
systems and discussed several aspects of such systems along with metrics $\g$,
which the designer is interested in evaluating. In \sref{interpolation}, we
obtained an efficient interpolation algorithm for approximating hypothetical
multidimensional functions $\f$.} We shall now amalgamate the ideas developed in
the aforementioned two sections.

\updated{Given an electronic system dependent on a number of uncertain
parameters $\vu: \outcomes \to \real^\nu$, the goal is to analyze a metric $\g$
...
\sref{power}. The goal is attained as follows; recall \fref{example}.
...
\updated{Since the expected value and variance, which are defined in
\eref{expectation} and \eref{variance}, respectively, usually draw particular
attention, we would like to elaborate on them separately.}
...
\updated{The solution is to let $h = (\g, \g^2)$ be the metric instead of $\g$.}
...
previous subsection: $h$ is vector valued. \updated{More generally, the metric
$\g$ in \sref{modeling} and the function $\f$ in \sref{interpolation} have been
depicted as having one-dimensional codomains.} This, however, has been done only
...
interpolant. \updated{The framework extends naturally to metrics with multiple
outputs, and it provides analytical formulae for expectations and variances.}
