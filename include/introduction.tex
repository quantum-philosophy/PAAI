...
system's behavior. Consider, for instance, \up{MC} sampling, which is arguably
...
on polynomial chaos (\up{PC}) expansions \cite{xiu2010} and other approximation
...
favors nondifferentiability and even discontinuity. In such cases, \up{PC}
...
First, we try to quantify the end-to-end delay by constructing a \up{PC}
expansion founded on the Legendre polynomial basis \cite{xiu2010}. The orange
line in \fref{motivation} shows a ninth-order \up{PC} expansion, which uses 10
...
studied, which can be also seen in \fref{motivation}. The magnitude of reduction
