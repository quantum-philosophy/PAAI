...
In order to account for uncertainty, one has to quantify it first. \updated{In
this setting, one is usually interested in evaluating a certain metric whose
complete knowledge would be highly profitable for the design at hand but cannot
be attained since the metric involves a number of parameters that are inherently
...
\updated{Uncertainty quantification is a broad area. The techniques for
uncertainty quantification can deliver radically different pieces of information
about the metric under consideration.} In this paper, we are interested in
...
apply. \updated{The system at hand is treated as an opaque object, and one only
has to evaluate this object a number of times in order to start to draw
conclusions about the system's behavior.}

\updated{The major problem with sampling techniques, however, is in sampling:
one should be able to obtain sufficient many realizations of the metric of
interest in order to accurately estimate the needed statistics about that metric
\cite{diaz-emparanza2002}.} The main concerns in this context are: \updated{How
expensive is it to obtain one sample in terms of the computation time? How many
samples are needed? How many samples are affordable?} When the subject under
...
need no change. \updated{Consequently, the metrics that the framework is able to
tackle are diverse. Examples include those metrics concerned with timing-,
power-, and temperature-related characteristics of elaborate applications
running on heterogeneous multiprocessor platforms.}

\updated{In contrast to sampling methods, our technique explores and exploits
the nature of the problem---that is, the way the metric depends on the uncertain
...
on the level on individual points \cite{ma2009}) variations.} This means that
...
nondifferentiability and even discontinuity. \updated{In such cases, \up{PC}
expansions and similar techniques fail: they require extremely many evaluations
of the desired metric in order to deliver an acceptable level of accuracy and,
hence, are not worth it.}

\updated{In order to illustrate this concern, let us consider an example.}
...
the application. \updated{In this example, the metric is the end-to-end delay,
and it coincides with the execution time of the task; hence, we already know the
...
expansion, which uses 10 points. \updated{It can be seen that the approximation
is poor---not to mention negative execution times---which means that the
follow-up sampling will also yield a poor approximation of the true
...
no particular attention. \updated{Having constructed such a representation, one
can proceed to the calculation of the probability distribution of the metric,
...
well suited for nonsmooth response surfaces. \updated{More generally, the
adaptivity featured by our technique allows for a reduction of the costs
associated with probabilistic analysis of the metric under consideration, as
measured by the number of times the metric needs to be evaluated in order to
achieve a certain accuracy level.} The magnitude of the reduction depends on the
problem, and it can be substantial when the problem is well disposed to
adaptation.

\updated{Lastly, we would like to reiterate that the output of our framework is
generative: we construct a light representation of the metric of interest that
can be used to generate realizations of this metric without touching the
...
provides an overview of the prior work. \updated{In \sref{present-work}, the
contribution of the paper is summarized. The preliminaries are given in
\sref{preliminaries}. The addressed problem is formulated in \sref{problem}, and
our solution is outlined in \sref{solution}. An illustrative example is
considered in \sref{example}. The framework is presented in \sref{modeling},
\ref{sec:interpolation}, and \ref{sec:analysis}. The experimental results are
given in \sref{experimentation}.} Section \ref{sec:conclusion} concludes the
paper.
