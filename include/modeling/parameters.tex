...
\begin{equation} \elab{transformation}
  \transformation: \real^\nu \to [0, 1]^\nz.
\end{equation}
\updated{For any point $\vz \in [0, 1]^\nz$, we are now able to compute the
corresponding $\vu$ and, consequently, the metric $\g$ as $(\g \circ
\transformation)(\vz) = \g(\transformation(\vz)) = \g(\vu)$; recall
\sref{problem} and see \fref{example}.}
...
\updated{Let us emphasize that this $\transformation$ is an example; the
framework works with any $\transformation$ that yields $\vz \sim
\distribution_\vz$ for some $\nz$.}
