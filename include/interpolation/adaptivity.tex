...
benefit from this peculiar behavior: we would treat both sides equally and would
...
interpolant, instead of evaluating the function at all possible nodes, we will
only choose those that are located in the regions with poor accuracy as
indicated by the yet-to-be-found accuracy metric.
...
to $\Delta\oindex_\vi$. This procedure is illustrated in \fref{grid} for the
open Newton--Cotes rule. The arrows emerging from a node connect the node with
its forward neighbors. It can be seen that each node has two forward neighbors
(for each dimension); their order indices are
\[
